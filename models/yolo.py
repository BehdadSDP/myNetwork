...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='XM-YOLOViT.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        super().__init__()
        ...
        self.init_anchor = False
        
        #instance of image Restoration 
...
'''Image Restoration'''
class ImageRst(nn.Module):

    def __init__(self, output_nc):
        super(ImageRst, self).__init__()
        #Input: 128*40*40
        # Upsampling
        ...
        # inputsize:64*80*80(concat feat2), outputsize:40*160*160
        self.conv_2 = nn.Sequential(
            nn.ConvTranspose2d(160, 40, 3, stride=2, padding=1, output_padding=1),
            nn.BatchNorm2d(40),
            nn.SiLU()
        )
        ...
        self.conv_3 = nn.Sequential(
            nn.ConvTranspose2d(104, 20, 3, stride=2, padding=1, output_padding=1),
            nn.BatchNorm2d(20),
            nn.SiLU()
        )
    ...
    def forward(self, x, y, z):
        """Standard forward"""
        # Decoding
        # batch size x 64 x 80 x 80
        c1 = self.conv_1(x)

        # batch size x 160 x 80 x 80
        skip1_de = torch.cat((c1, y), 1)

        # batch size x 40 x 160 x 160
        c2 = self.conv_2(skip1_de)

        # batch size x 104 x 160 x 160
        skip2_de = torch.cat((c2, z), 1)

        # batch size x 20 x 320 x 320
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, ConvLayer,
                InvertedResidual}:
            c1, c2 = ch[f], args[0]
